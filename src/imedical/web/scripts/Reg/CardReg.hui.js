...
function Init() {
    ...
    PageLogicObj.m_MaleTrans = tkMakeServerCall("DHCDoc.Common.Translate", "GetTransSex", "男")
    PageLogicObj.m_FemaleTrans = tkMakeServerCall("DHCDoc.Common.Translate", "GetTransSex", "女")
}
function InitBtnBar()
{
	$('#btnList').marybtnbar({
...
function setBirthAndSex(mypId) {
    ...
    if (myary[0] == "1") {
        ...
        var mySexDR = "";
        switch ($g(myary[3])) {
            case PageLogicObj.m_MaleTrans:
                mySexDR = "1";
                break;
            case PageLogicObj.m_FemaleTrans:
                mySexDR = "2";
                break;
            default:
                mySexDR = "4";
    ...
    if (myary[0] == '0') {
        ...
        switch (myary[0]) {
            ...
            case "-351":
                ...
                $.messager.alert('提示', $g("此卡已经被挂失,不能使用,挂失人:") + CancelInfo.split("^")[0] + "," + $g("挂失原因:") + $g(CancelInfo.split("^")[1]));
                break;
            case "-352":
...
function SaveDataToServer() {
    ...
    new Promise(function(resolve, rejected) {
        ...
        return new Promise(function(resolve, rejected) {
			...
			myExpStr=myExpStr+"^"+PageLogicObj.m_PatExceptionKey
            var rtn = $.cm({
                ClassName: "web.DHCBL.CARDIF.ICardRefInfo",
...
function CheckData() {
        ...
        if (PageLogicObj.m_UsePANoToCardNO != "Y") {
            if (myCardNo == "") {
                ...
                if ((PageLogicObj.m_CardNoLength != 0) && (myCardNo.length != PageLogicObj.m_CardNoLength)) {
                    $.messager.alert("提示", $g("卡号长度应该为") + PageLogicObj.m_CardNoLength + $g("位") + "，" + $g("请核实！"), "info", function() {
                        $("#CardNo").focus();
                    });
                    return false;
                }
    ...
    if ((InsuNo != "") && (InsuNo != "99999999999S")) {
        ...
        if (Rtn > 0) {
            $.messager.alert("提示", InsuNo + $g("医保号已被使用!"), "info", function() {
                $("#PatYBCode").focus();
            });
            return false
        }
    ...
    if (PageLogicObj.m_PatMasFlag == "Y") {
        ...
        if (myBirth != "") {
            ...
            if ((ageStr.split("||")[1] != myage) && (ageStr.split("||")[0] != myage)) {
                if (ageStr.split("||")[1] != 0) {
                    $.messager.alert("提示", $g("出生日期不能与年纪不相等!实际年龄") + ageStr.split("||")[1], "info", function() {
                        $("#Birth").focus();
                    });
                    return false;
                } else {
                    $.messager.alert("提示", $g("出生日期不能与年纪不相等!实际年龄") + ageStr.split("||")[0], "info", function() {
                        $("#Birth").focus();
                    });
                    return false;
                }
    ...
    if (PageLogicObj.m_PatMasFlag == "Y") {
        ...
        SexData.forEach(object =>{
            if(object.id === mySexID){
                if (object.code ==1){
                    SexDesc=PageLogicObj.m_MaleTrans;
                }else{
                    SexDesc=PageLogicObj.m_FemaleTrans;
                }
            }
        });
        var myPAPERMarital = $("#PAPERMarital").combobox('getValue');
        var AgeToMaritalFlag = 0
        if (SexDesc == PageLogicObj.m_FemaleTrans) {
            if ((Age < PageLogicObj.m_MarriedLimitFemaleFAge) && (("^" + PageLogicObj.m_MarriedIDStr + "^").indexOf("^" + myPAPERMarital + "^") != -1)) {
                AgeToMaritalFlag = 1;
            }
        } else if (SexDesc == PageLogicObj.m_MaleTrans) {
        ...
        if (myIDNo != "") {
            ...
            if (myIDrtn) {
                ...
                if (mySameFind == "1") {
                    $.messager.alert("提示", myIDNo + " " + $g("此证件号码已经被使用!"), "info", function() {
                        $("#CredNo").focus();
                    })
                    return false;
                }
...
function PatInfoUnique() {
    ...
    if (RtnArr[0] == "0") {
        ...
        $.messager.alert('提示', $g('此卡类型、姓名、性别、出生日期、联系电话信息绑定已挂失卡') + RtnArr[1] + ',' + $g('请作废此卡后重新建卡'));
        return false;
    } else if (RtnArr[0] == "N") {
        $.messager.alert('提示', $g('此卡类型、姓名、性别、出生日期、联系电话信息绑定卡') + RtnArr[1] + ',' + $g('请办理其他卡或者补卡'));
        return false;
    }
    return true;
}
...
function GetPatMasInfo() {
    ...
    if (PageLogicObj.m_PatMasFlag == "Y") {
        ...
        var myxml = GetEntityClassInfoToXML(myparseinfo,"Pat")
    }
    return myxml;
}
...
function GetEntityClassInfoToXML(ParseInfo,Type) {
    var myxmlstr = "";
    try {
        ...
        for (var myIdx = 1; myIdx < myary.length; myIdx++) {
            ...
            if (_$id.length == 0) {
                ...
                if (Type =="Pat"){
	                PageLogicObj.m_PatExceptionKey =PageLogicObj.m_PatExceptionKey +String.fromCharCode(1)+myary[myIdx]
	                }
            }
...
function CardSearchDBClickHander(row) {
    ...
    if (CardNO.indexOf(",") < 0) {
        ...
        // 翻译卡类型有空格
        var CardNoArr = CardNO.split(" ");
        var CardNo = CardNoArr[CardNoArr.length-1];
        var IsTemporaryCard = InitTemporaryCard(CardNo);
        if (IsTemporaryCard == "Y") {
            $("#CardNo").val(CardNo);
        }
    ...
    if (myComIdx >= 0) {
        if (myComIdx == (PageLogicObj.m_CardRegJumpSeqArr.length - 1)) {
            ...
            if (id != "undefined") {
                ...
                if ((_$id.closest('tr.baseinfo').length > 0)&&(_$id.closest('tr.baseinfo').css('display') == "none")) {
                    DOMFocusJump(id)
                    return
                }
                if ((_$id.closest('tr.addressinfo').length > 0)&&(_$id.closest('tr.addressinfo').css('display') == "none")) {
                    DOMFocusJump(id)
                    return
                }
                //if (_$id.hasClass("hisui-combobox")){
...
function SyncMotherInfoClick(){
	var PAPMIRowID = $("#PAPMIRowID").val();
    if (PAPMIRowID == "") {
        $.messager.alert("提示", "请选择需要同步的记录!");
        return false;
    }
    var ret = $.cm({
        ClassName: "web.DHCBL.CARDIF.ICardPaPatMasInfo",
        MethodName: "GetMotherInfo",
        BabyPersonID: PAPMIRowID,
        dataType: "text"
    }, false);
    if (ret==""){
	    $.messager.alert("提示", "无需要关联的母亲信息记录!");
        return false;
	}
	var MotherInfoArr=ret.split("^")
	//$("#PatType").combobox("select",MotherInfoArr[0]);
	$("#TelHome").val(MotherInfoArr[1]);
	$("#ForeignPhone").val(MotherInfoArr[1]);
	if (PageLogicObj.m_IsNotStructAddress == "Y") {
        $("#Address").combobox('setText', MotherInfoArr[2]);
    } else {
        $("#Address").val(MotherInfoArr[2]);
    }
    $("#ForeignAddress").val(MotherInfoArr[2]);
    $("#ForeignName").val(MotherInfoArr[3]);
    $("#ForeignIDCard").val(MotherInfoArr[4]);
    //$("#ForeignCredType").combobox("select",MotherInfoArr[5]);
    var Data = $("#ForeignCredType").combobox("getData");
    for (var m = 0; m < Data.length; m++) {
        var id = Data[m]["id"];
        if (MotherInfoArr[5] == id.split("^")[0]) {
            $("#ForeignCredType").combobox("select", id);
            break;
        }
    }                  
    $("#CTRelationDR").combobox("select",MotherInfoArr[6]);
    $("#CountryDescLookUpRowID").combobox('select', MotherInfoArr[7])
}
