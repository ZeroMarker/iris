...
function UpdateclickHandler(type){
	...
	if (PayModeName=="") PayModeName= $g("原支付方式")
	$.messager.confirm('确认对话框', $g('是否确定要退号?')+"<div>"+$g("退回")+PayModeName+":<div style='color:#f16e57'>"+CostSum+$g("元")+"</div></div>"+"<div>"+$g("原支付方式")+":<div style='color:#2ab66a'>"+PayModeStr+"</div></div>", function(r){
		if (r){
			var ReturnMR="N"
			var o=$HUI.checkbox("#ReturnMR");
			if (o.getValue()){
